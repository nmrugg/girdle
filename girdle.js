                ...
                return function create_easy_ajax()
                {
                    ...
                    ajax_obj = {
                        ...
                        query: (function ()
                        {
                             ...
                             * @param   onresponse(function) (optional) The function to run when the response is in (success or failure).
                             ...
                             */
                            return function query(method, path, message, onresponse, timeout, retry)
                            {
                                var post_message,
                                    failed;
                                ...
                                retry_func = function ()
                                {
                                    ...
                                    retrying = false;
                                    query(method, path, message, onresponse, timeout, retry);
                                };
                                ...
                                {
                                    var options = {retry: retry};
                                    
                                    if (onresponse) {
                                        onresponse(parse_if_json(ajax), ajax.status, options);
                                    }
                                ...
                                ajax.onload = function onload()
                                {
                                    ...
                                    if (ajax.status === 200) {
                                        if (onresponse) {
                                            onresponse(parse_if_json(ajax), ajax.status);
                                        }
                                    } else {
                                        failed();
                                    }
